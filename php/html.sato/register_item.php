...
<html lang ="ja">
    ...
    <body>
        ...
        <?php
            ...
            // echo'<td><textarea name="about" placeholder="商品説明"></textarea></td>';
            // echo'<td><input type="text" name="maker">';
            // echo'<td><input type="text" name="cost"></td>';
            // echo'<td><input type="text" name="selling_price"></td>';
            // echo'</tr>';
            //echo'</table>';
            ...
            else if (isset($_POST["func"]) && $_POST["func"] == "upd_data") 
            {
                ...
                if ($upddata) {
                    for ($i=0; $i<count($upddata); $i++) {
                        $sql = "UPDATE items SET number=?, type=?,about=?,maker=?,cost=?,selling_cost=? WHERE id=?";
                        $stmt = $db->prepare($sql);
                ...
                if ($insdata) {
                    for ($i=0; $i<count($insdata); $i++) {
                        $sql = "INSERT INTO items (name, age) VALUES (?, ?)";
                        $stmt = $db->prepare($sql);
